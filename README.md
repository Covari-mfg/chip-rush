...
Every role receives one optional Wire EDM job at 35 seconds. It uses no regular order slot. Choose **Outsource with Covari**, then click the Office computer and spend two attended seconds placing it. The real Covari logo appears on the bonus card and the computer while placing the job. This bonus is available in Operator, Production Manager and Owner. A partner delivers 22 seconds later for 60 bonus points in any role, below normal machining rewards. Passing or ignoring the offer has no penalty. Sourcing never adds shipments, stars or streak credit, and calls pause approval. This is a game simulation; it does not submit a real sourcing request.
